...
for i in range(0,u):
    ...
    port=8671+i%u
    file.write("#!/bin/bash\n")
    file.write(p0+p1+usrs[i%u]+p2+usrs[(i%u+2)%u]+p3+str(port)+p4)
    #file.write(p1+usrs[i%5]+p2+usrs[(i%5+1)%5]+p3+str(port)+p4)
    file.write(p5)
    file.close()

for i in range(0,s):
    dir = '/data/xr/testchain/sends/send'+str(i+u)+'.sh'
    file=open(dir,'w+')
    p0 ='for ((i=1;i<500;i++)); do  echo $i; \n'
    p1='time curl --data \'{"method":"eth_sendTransaction","params":[{"from":"'
    p2='","to":"'
    ...
    port=8650+i%s
    file.write("#!/bin/bash\n")
    file.write(p0+p1+sealers[i%s]+p2+sealers[(i%s+2)%s]+p3+str(port)+p4)
    #file.write(p1+usrs[i%5]+p2+usrs[(i%5+1)%5]+p3+str(port)+p4)
    file.write(p5)
    file.close()

file=open('/data/xr/testchain/send.sh','w+')
for i in range(0,u+s):
