...
impl Step {
    ...
    fn increment(&self) {
        ...
        // info point 1, step.inner
        //info!("---- self.inner increased, new = {:#?} ----",self.inner);
    }
    ...
    fn check_future(&self, given: u64) -> Result<(), Option<OutOfBounds<u64>>> {
        ...
        // wait a bit for blocks in near future
            ...
            info!("------ check future(), given = {}, current = {} --------",given,current);
            let d = self
                .durations
                .iter()
    ...
    fn timeout(&self, io: &IoContext<()>, timer: TimerToken) {
        if timer == ENGINE_TIMEOUT_TOKEN {
            ...
            while AsMillis::as_millis(&self.step.inner.duration_remaining()) == 0 {
                ...
                info!("####### can_propose lock is now set to TRUE ########");
...
impl Engine<EthereumMachine> for AuthorityRound {
    ...
    fn populate_from_parent(&self, header: &mut Header, parent: &Header) {
        ...
        info!("^^^^^^ calculate_score: author = {:?}, current_empty_steps_len = {}",header.author(),current_empty_steps_len);
    ...
    fn is_epoch_end(
        ...
        for finalized_hash in finalized {
            if let Some(pending) = transition_store(*finalized_hash) {
                ...
                info!("####### can_propose lock is now set to FALSE ########");
