...
pub trait Engine<M: Machine>: Sync + Send {
    ...
    // *** malicious 
    fn open_block_header_timestamp1(&self, parent_timestamp: u64) -> u64 {
        use std::{cmp, time};
        ...
        use chrono::{Utc, Local, NaiveDate, NaiveDateTime};
        //let twenty = Duration::new(20, 0);
        let t25 = Duration::new(25, 0);
        let malicious_time = parent_timestamp + t25.as_secs();
        let malicious_systime = SystemTime::UNIX_EPOCH.checked_add(Duration::new(malicious_time, 0)).unwrap();
        let datetime1: DateTime<Local> = SystemTime::now().into();
        let datetime2: DateTime<Local> = malicious_systime.into();
        let now_str = datetime1.format("%Y-%m-%d %H:%M:%S.%f").to_string();
        let malicious_str = datetime2.format("%Y-%m-%d %H:%M:%S.%f").to_string();
        info!(">>>>>>> producing malicious block, acual time = {}, altered time = {} >>>>>>>>>>>", now_str, malicious_str);
        malicious_time
        //cmp::max(now.as_secs() as u64, parent_timestamp + 1)
    }
