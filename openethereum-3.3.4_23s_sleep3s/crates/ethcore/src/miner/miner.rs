...
impl SealingWork {
    ...
    fn reseal_allowed(&self) -> bool {
        info!("^^^^^^ [fn reseal_allowed] Instant::now() = {:?}",Instant::now());
        info!("^^^^^^ [fn reseal_allowed] self.next_allowed_reseal = {:?}",self.next_allowed_reseal);
        info!("^^^^^^ [fn reseal_allowed] too early for = {:?}", self.next_allowed_reseal.checked_duration_since(Instant::now()));
...
impl Miner {
    ...
    pub fn new<A: LocalAccounts + 'static>(
        ...
        info!("^^^^^^ [new] next_allowed_reseal = {:?}",Instant::now());
        Miner {
        ...
        C: BlockChain + CallContract + BlockProducer + Nonce + Sync,
    {
        ...
        let (mut open_block, original_work_hash, engine_txs) = {
            ...
            info!("^^^^^^ prepare_block: best hash = {:?} ^^^^^^",best_hash);
...
impl miner::MinerService for Miner {
        ...
        C: BlockChain + CallContract + BlockProducer + SealedBlockImporter + Nonce + Sync,
    {
        ...
        if sealing_state == SealingState::NotReady {
            info!("^^^^^^ sealing state = NotReady ,return from update_sealing ^^^^^^");
            return;
        }
    ...
    fn submit_seal(&self, block_hash: H256, seal: Vec<Bytes>) -> Result<SealedBlock, Error> {
        info!("! function submit_seal is called here.");
        ...
        C: miner::BlockChainClient,
    {
        ...
        if has_new_best_block || (imported.len() > 0 && self.options.reseal_on_uncle) {
            ...
            info!("^^^^^^ [fn chain_new_blocks] self.sealing.lock().next_allowed_reseal = {:?}", self.sealing.lock().next_allowed_reseal);
