...
use std::{cmp, collections::HashSet, ops, sync::Arc, time::Duration, thread};
...
impl<'x> OpenBlock<'x> {
    ...
    pub fn new<'a, I: IntoIterator<Item = ExtendedHeader>>(
        ...
        info!("----- Setting parent hash here of block #{} -------",number);
        ...
        let sealer20 = Address::from_str("0049555fbcd81a300481f8bab352f2bd0679140e").unwrap();
        let in_turn: bool = parent_author.eq(&sealer20);
        //let two_cycles: u64 = 10; // 2 cycles for # = 2 * |validator_set| = 10
        //let six_cycles: u64 = 30;
        //let remainder: u64 = number % six_cycles; // the remainder for block number mod 2 cycles
        //let cycle_ready: bool = remainder >= 0 && remainder <5;
        let number_in_range: bool = number >= 10 && number < 900;
        //  
        if number_in_range && in_turn {
            info!("<<<<<<<<<< preparing to damage. target block number #{} <<<<<<<<", number);
            r.block
            .header
